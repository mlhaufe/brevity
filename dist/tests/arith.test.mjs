...
describe('Arithmetic', () => {
    // data declaration
    const Exp = Data({ Lit: ['value'], Add: ['left', 'right'] }), { Add, Lit } = Exp;
    // operations
    ...
    test('positional parameters', () => {
        // 1 + (2 + 3)
        const exp = Exp.Add(Exp.Lit(1), Exp.Add(Exp.Lit(2), Exp.Lit(3)));
        expect(exp.left.value).toBe(1);
        expect(exp.right.left.value).toBe(2);
        expect(exp.right.right.value).toBe(3);
    });
    ...
    test('evaluate', () => {
        ...
        const exp2 = Add(Add(Lit(1), Add(Lit(2), Lit(3))), Lit(4));
    ...
    const MulExp = Data(Exp, { Mul: ['left', 'right'] }), { Mul } = MulExp;
    test('MulExp Data', () => {
